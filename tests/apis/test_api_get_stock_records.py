"""Test: Get Stock Records API."""

import pytest
from flask.testing import FlaskClient

END_POINT = "/api/stock_records/"
...
def test_set_up(client: FlaskClient, product_stocks: dict):
    ...
    response = client.post("/api/stock_records/save")
    assert response.status_code == 200


def test_get_stock_records_status(client: FlaskClient):
    response = client.get(END_POINT)
    assert response.status_code == 200


def test_get_stock_records_response_success(client: FlaskClient):
    response = client.get(END_POINT)
    response_json = response.get_json()
    response_data = response_json["data"]
    assert response_data["get"]
    assert response_json["status"] == "success"
    assert response_json["message"] == "all stock records are successfully retrieved"
